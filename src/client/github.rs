...
#[graphql(
  ...
  query_path = "graphql/query/Query.graphql",
...
fn parse_repo_name(repo_name: &str) -> Result<(&str, &str), anyhow::Error> {
  ...
  match (parts.next(), parts.next()) {
    ...
    _ => Err(format_err!(
      "wrong format for the repository name param (we expect something like facebook/graphql)"
    )),
  }
}
...
fn main() -> Result<(), anyhow::Error> {
  ...
  let response_body =
    post_graphql::<ViewRepo, _>(&client, "https://api.github.com/graphql", variables).unwrap();
